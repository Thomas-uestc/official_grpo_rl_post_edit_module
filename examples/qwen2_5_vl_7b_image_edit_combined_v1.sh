...
PHYSICAL_GEOMETRIC_WEIGHT=0.25
ENVIRONMENT_CONTEXT_WEIGHT=0.25
CULTURAL_SOCIAL_WEIGHT=0.05
LOGICAL_CAUSAL_WEIGHT=0.10
TARGET_ATTRIBUTION_WEIGHT=0.25
RULE_FORMAT_WEIGHT=0.10

python3 -m verl.trainer.main \
    config=examples/image_edit_config_combined.yaml \
    trainer.project_name=post_reasoning_edit_easyr1_combined_reward \
    trainer.experiment_name=experiment_test_test \
    ...
    worker.reward.combined_strategy=weighted_sum \
    worker.reward.combined_gpt_physical_geometric_weight=${PHYSICAL_GEOMETRIC_WEIGHT} \
    worker.reward.combined_gpt_environment_context_weight=${ENVIRONMENT_CONTEXT_WEIGHT} \
    worker.reward.combined_gpt_cultural_social_weight=${CULTURAL_SOCIAL_WEIGHT} \
    worker.reward.combined_gpt_logical_causal_weight=${LOGICAL_CAUSAL_WEIGHT} \
    worker.reward.combined_gpt_target_attribution_weight=${TARGET_ATTRIBUTION_WEIGHT} \
    worker.reward.combined_rule_format_weight=${RULE_FORMAT_WEIGHT} \
